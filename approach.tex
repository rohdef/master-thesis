...
(Section~\ref{sec:problems_structural_conjunctural}), the structural
approach to the issues is desired.  This choice means to ensure that
...
might find it good that it is enforced.  Updating or replacing
{\bibtex} to handle the issues does not ensure separation of concerns.
...
\item For duplicate entries having unique identifiers such as arXiv
  ...
  should be detected.  For textual values it should compare the values
  that are likely to be repetitive.  Thus solving the lexical concerns
  will be of use.
...
resources, such as DBLP~\cite{bibtex2006_dblp}, where both versions of
the title is arguably correct.
