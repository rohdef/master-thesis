...
will arise, sooner or later, \file{bib}s being no exception.  Extracting
...
to the bad look ups would be if it was possible to detect the user's
...
as if the correct names are written, how conjunctural depends on the
correctness of the tools.  Bibliography managers (see
...
the issue, as no such tools exist (to the author's knowledge).
...
Provided that a solution for the duplicate entry issue is already in
...
\item Conformity to de-facto standards and the {\bibtex} specification
  is desired.  A structural solution requires something that checks
  the conformity to a combination of specifications and de-facto
  standards accounting for changes in de-facto standards.
...
\item Inconsistent tag usage in similar entries causes messy
  ...
  inconsistencies and will be able so suggest a course of action.  The
