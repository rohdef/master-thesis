The goal of this chapter is to show that there is a lot of tools and
ways to manage bibliographies and how these tools tries to handle the
before mentioned issues, if they do so.
...
organized as follows: \chapref{sec:bibtex_tools} contains an overview
of the pure tools for {\bibtex}, \chapref{sec:bibtex_alternatives}
contains some of the attempts to replace {\bibtex} with similar
formats, and \chapref{sec:bibliography_managers} strives to
...
directly targeted at {\bibtex}.  Some of these provides partial
...
capitalization inconsistencies and bulk DOI
...
%interest, such as inconsistencies and spelling errors, in {\bibtex}
...
%Abbreviation detection in {\bibtex} will usually only have one
...
%occur.  Thus a clustering based system like in~\cite{okazaki2010_building} will be overkill.  It is possible that a
...
%located close to it at one of its uses.  However in {\bibtex} it is
...
{\bibtex} and reference managers that strive to replace the entire
...
{\bibtex} tools work on that too (this has not been tested).  The
